...
class FacialService:
    ...
    def _guardar_foto_si_incidencia(
        ...
        """
        Si el escaneo recién insertado generó una incidencia (la crea el trigger de
        la BD, p. ej. 'fuera_de_area'), guarda el recorte del rostro como JPEG en
        disco y setea `incidencias.ruta_foto`. Si no hubo incidencia, no hace nada.

        Llamar DESPUÉS de _registrar_escaneo (commit), para que el trigger ya haya
        corrido y la incidencia exista.
        """
        incidencias = (
            db.query(Incidencia)
    ...
    def procesar_frame_acceso(
        ...
        # Si el trigger marcó este escaneo como incidencia, conserva la foto del rostro.
