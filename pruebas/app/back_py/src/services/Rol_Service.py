...
class RolService:
    ...
    def registrar_rol(self, datos: RolCreate, db: Session) -> Rol:
        """Crea un rol. nombre_rol es único."""
        rol = Rol(
            nombre_rol=datos.nombre_rol,
            descripcion=datos.descripcion,
            permisos=datos.permisos.model_dump(),  # JSONB espera dict, no el modelo
            estado=datos.estado,
        )
        db.add(rol)
        try:
    ...
    def listar_roles(self, db: Session, skip: int = 0, limit: int = 100) -> list[Rol]:
        """Devuelve los roles registrados, ordenados por id."""
        return (
            db.query(Rol)
            .order_by(Rol.id_rol)
            .offset(skip)
    ...
    def obtener_rol(self, id_rol: int, db: Session) -> Rol:
        """Devuelve un rol por su id o lanza 404 si no existe."""
    ...
    def actualizar_rol(self, id_rol: int, datos: RolUpdate, db: Session) -> Rol:
        """Actualiza parcialmente un rol (solo los campos enviados)."""
    ...
    def eliminar_rol(self, id_rol: int, db: Session) -> Rol:
        """Baja lógica (soft delete): marca el rol como 'inactivo'."""
        rol = self.obtener_rol(id_rol, db)
        rol.estado = "inactivo"
        db.commit()
        db.refresh(rol)
