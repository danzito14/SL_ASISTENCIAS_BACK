...
class EscaneoService:
    ...
    def registrar_escaneo(self, datos: EscaneoCreate, db: Session) -> Escaneo:
        ...
        y se guarda como geography(POINT,4326).
        """
        escaneo = Escaneo(
    ...
    def listar_escaneos(
        ...
        Opcionalmente se filtran por trabajador y/o empresa (vía Trabajador->Área).
        """
        ...
        if id_empresa is not None:
            query = (
                query.join(Trabajador, Trabajador.id_trabajador == Escaneo.id_trabajador)
    ...
    def obtener_escaneo(self, id_escaneo: int, db: Session) -> Escaneo:
    ...
    def actualizar_escaneo(self, id_escaneo: int, datos: EscaneoUpdate, db: Session) -> Escaneo:
        """
    ...
    def eliminar_escaneo(self, id_escaneo: int, db: Session) -> Escaneo:
        """
    ...
    def empresa_de_escaneo(self, id_escaneo: int, db: Session) -> int | None:
        """Empresa de un escaneo (vía Escaneo->Trabajador->Área), o None si no existe."""
        fila = (
            db.query(AreaTrabajo.id_empresa)
            .join(Trabajador, Trabajador.id_area == AreaTrabajo.id_area)
            .join(Escaneo, Escaneo.id_trabajador == Trabajador.id_trabajador)
