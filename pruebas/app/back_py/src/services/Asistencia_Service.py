...
class AsistenciaService:
    ...
    def obtener_asistencia(self, id_asistencia: int, db: Session) -> Asistencia:
        """
    ...
    def empresa_de_asistencia(self, id_asistencia: int, db: Session) -> int | None:
        """Empresa de una asistencia (vía Asistencia->Trabajador->Área), o None si no existe."""
        fila = (
            db.query(AreaTrabajo.id_empresa)
            .join(Trabajador, Trabajador.id_area == AreaTrabajo.id_area)
            .join(Asistencia, Asistencia.id_trabajador == Trabajador.id_trabajador)
