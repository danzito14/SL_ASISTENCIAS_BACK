...
class DispositivosService:
    ...
    def listar_dispositivos(
        ...
        if id_empresa is not None:
            query = query.join(
                AreaTrabajo, Dispositivo.id_area == AreaTrabajo.id_area
    ...
    def empresa_de_dispositivo(self, id_dispositivo: int, db: Session) -> int | None:
        """Empresa de un dispositivo (vía su área), o None si no existe."""
        fila = (
            db.query(AreaTrabajo.id_empresa)
            .join(Dispositivo, Dispositivo.id_area == AreaTrabajo.id_area)
