...
Solo protege el API de gestión de cámaras/eventos (rol 'vigilancia' con scopes
vigilancia:read/write). El motor de captura NO pasa por aquí.
"""
...
# Rutas públicas (para el guard de SCOPES). El login/token vive en identity, no aquí.
# /vigilancia/reid/config no la consume el front (la pide el supervisor reid): se salta
# el scope y se protege con su propio token interno (X-Reid-Token) dentro del endpoint.
RUTAS_PUBLICAS = {
    "/", "/health", "/docs", "/redoc", "/openapi.json", "/docs/oauth2-redirect",
    "/vigilancia/reid/config",
    "/vigilancia/reid/avistamiento",
    "/vigilancia/reid/identificar",
}
...
def _verificar_gateway(request: Request) -> None:
    ...
    y NO es un acceso directo al contenedor. Vacío = no se exige.
    """
...
def guard_scopes(request: Request) -> None:
    ...
    # DEV/PRUEBA: sin auth. Inyecta un principal super-admin. NUNCA en producción.
