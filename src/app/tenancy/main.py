# tenancy/main.py — estructura física: empresas, áreas, puertas, dispositivos.
import logging
...
async def lifespan(app: FastAPI):
    try:
        ...
        logger.info("tenancy: conexión a la BD OK (%s como %s).", settings.DB_NAME, settings.DB_USER)
    except Exception as exc:
        logger.error("tenancy: no se pudo conectar a la BD: %s", exc)
        raise RuntimeError("tenancy: sin conexión a la base de datos.") from exc
    yield
...
def health():
    try:
        ...
        logger.warning("tenancy healthcheck: BD no disponible: %s", exc)
        db_ok = False
    payload = {"status": "ok" if db_ok else "degraded", "service": "tenancy",
